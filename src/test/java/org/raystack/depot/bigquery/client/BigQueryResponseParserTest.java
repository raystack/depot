...
public class BigQueryResponseParserTest {
        ...
        public void shouldParseResponse() {
                ...
                Map<Long, ErrorInfo> errorInfoMap = BigQueryResponseParser.getErrorsFromBQResponse(records, response,
                                metrics,
                                instrumentation);

                Assert.assertEquals(new ErrorInfo(new BigQuerySinkException(), ErrorType.SINK_UNKNOWN_ERROR),
                                errorInfoMap.get(0L));
                Assert.assertEquals(new ErrorInfo(new BigQuerySinkException(), ErrorType.SINK_4XX_ERROR),
                                errorInfoMap.get(1L));
                Assert.assertEquals(new ErrorInfo(new BigQuerySinkException(), ErrorType.SINK_4XX_ERROR),
                                errorInfoMap.get(2L));
                Assert.assertEquals(new ErrorInfo(new BigQuerySinkException(), ErrorType.SINK_5XX_ERROR),
                                errorInfoMap.get(3L));

                Mockito.verify(instrumentation, Mockito.times(1)).incrementCounter("test",
                                String.format(BigQueryMetrics.BIGQUERY_ERROR_TAG,
                                                BigQueryMetrics.BigQueryErrorType.UNKNOWN_ERROR));
                Mockito.verify(instrumentation, Mockito.times(1)).incrementCounter("test", String
                                .format(BigQueryMetrics.BIGQUERY_ERROR_TAG,
                                                BigQueryMetrics.BigQueryErrorType.INVALID_SCHEMA_ERROR));
                Mockito.verify(instrumentation, Mockito.times(1)).incrementCounter("test",
                                String.format(BigQueryMetrics.BIGQUERY_ERROR_TAG,
                                                BigQueryMetrics.BigQueryErrorType.OOB_ERROR));
                Mockito.verify(instrumentation, Mockito.times(1)).incrementCounter("test",
                                String.format(BigQueryMetrics.BIGQUERY_ERROR_TAG,
                                                BigQueryMetrics.BigQueryErrorType.STOPPED_ERROR));
        }
}
