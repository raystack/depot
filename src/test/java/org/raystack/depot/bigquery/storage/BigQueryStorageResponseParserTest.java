...
public class BigQueryStorageResponseParserTest {
    ...
    public void shouldReturnErrorFromStatus() {
        com.google.rpc.Status status = com.google.rpc.Status.newBuilder().setCode(Code.PERMISSION_DENIED_VALUE)
                .setMessage("test error").build();
    ...
    public void shouldSetErrorResponse() {
        ...
        payload.addMetadataRecord(new BigQueryRecordMeta(1,
                new ErrorInfo(new Exception("error1"), ErrorType.DESERIALIZATION_ERROR), false));
        payload.addMetadataRecord(new BigQueryRecordMeta(2, null, true));
        payload.addMetadataRecord(new BigQueryRecordMeta(3,
                new ErrorInfo(new Exception("error2"), ErrorType.UNKNOWN_FIELDS_ERROR), false));
        payload.addMetadataRecord(new BigQueryRecordMeta(4,
                new ErrorInfo(new Exception("error3"), ErrorType.INVALID_MESSAGE_ERROR), false));
    ...
    public void shouldSetResponseForError() {
        ...
        List<Message> messages = createMockMessages();
        AppendRowsResponse appendRowsResponse = AppendRowsResponse.newBuilder().setError(
                com.google.rpc.Status.newBuilder().setMessage("test error").setCode(Code.UNAVAILABLE_VALUE).build())
                .build();
    ...
    public void shouldSetResponseForRowError() {
        ...
        AppendRowsResponse appendRowsResponse = AppendRowsResponse.newBuilder()
                .setError(com.google.rpc.Status.newBuilder().setMessage("test error").setCode(Code.UNAVAILABLE_VALUE)
                        .build())
    ...
    public void shouldSetSinkResponseForException() {
        ...
        Assert.assertEquals("io.grpc.StatusRuntimeException: INTERNAL",
                response.getErrors().get(0L).getException().getMessage());
        Assert.assertEquals("io.grpc.StatusRuntimeException: INTERNAL",
                response.getErrors().get(1L).getException().getMessage());
        Assert.assertEquals("io.grpc.StatusRuntimeException: INTERNAL",
                response.getErrors().get(2L).getException().getMessage());
        Assert.assertEquals("io.grpc.StatusRuntimeException: INTERNAL",
                response.getErrors().get(3L).getException().getMessage());
        Assert.assertEquals("io.grpc.StatusRuntimeException: INTERNAL",
                response.getErrors().get(4L).getException().getMessage());
    }
    ...
    public void shouldSetSinkResponseForExceptionWithNonRetry() {
        ...
        Assert.assertEquals("io.grpc.StatusRuntimeException: RESOURCE_EXHAUSTED",
                response.getErrors().get(0L).getException().getMessage());
        Assert.assertEquals("io.grpc.StatusRuntimeException: RESOURCE_EXHAUSTED",
                response.getErrors().get(1L).getException().getMessage());
        Assert.assertEquals("io.grpc.StatusRuntimeException: RESOURCE_EXHAUSTED",
    ...
    public void shouldSetSinkResponseForExceptionWithAppendError() {
        ...
        Assert.assertEquals(
                "com.google.cloud.bigquery.storage.v1.Exceptions$AppendSerializationError: UNKNOWN: test error",
                response.getErrors().get(3L).getException().getMessage());
