...
public class BigQuerySinkTest {
    ...
    public void shouldPushToBigQuerySink() {
        ...
        TestMetadata record6Offset = new TestMetadata("topic1", 6, 104, Instant.now().toEpochMilli(),
                Instant.now().toEpochMilli());
        Message message1 = TestMessageBuilder.withMetadata(record1Offset).createConsumerRecord("order-1", "order-url-1",
                "order-details-1");
        Message message2 = TestMessageBuilder.withMetadata(record2Offset).createConsumerRecord("order-2", "order-url-2",
                "order-details-2");
        Message message3 = TestMessageBuilder.withMetadata(record3Offset).createConsumerRecord("order-3", "order-url-3",
                "order-details-3");
        Message message4 = TestMessageBuilder.withMetadata(record4Offset).createConsumerRecord("order-4", "order-url-4",
                "order-details-4");
        Message message5 = TestMessageBuilder.withMetadata(record5Offset).createConsumerRecord("order-5", "order-url-5",
                "order-details-5");
        Message message6 = TestMessageBuilder.withMetadata(record6Offset).createConsumerRecord("order-6", "order-url-6",
                "order-details-6");
