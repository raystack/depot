...
import io.odpf.sink.connectors.bigquery.converter.RowMapper;
...
@Slf4j
public class ProtoUpdateListener extends OdpfStencilUpdateListener {
    ...
    private void addMetadataFields(List<Field> bqSchemaFields) {
        ...
        if (config.shouldAddMetadata()) {
            ...
            if (namespaceName.isEmpty()) {
                bqMetadataFields.addAll(MetadataUtil.getMetadataFields(metadataColumnsTypes));
            } else {
                bqMetadataFields.add(MetadataUtil.getNamespacedMetadataField(namespaceName, metadataColumnsTypes));
            }
        }
