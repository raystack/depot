...
public class BigQueryStorageResponseParser {
    private static final Set<Code> RETRYABLE_ERROR_CODES = new HashSet<Code>() {
        {
            add(Code.INTERNAL);
            add(Code.ABORTED);
            add(Code.CANCELLED);
    ...
    public static AppendRowsResponse get4xxErrorResponse() {
        return AppendRowsResponse.newBuilder()
                .setError(Status.newBuilder().setCode(com.google.rpc.Code.FAILED_PRECONDITION.ordinal()).build())
                .build();
    }
    ...
    public void setSinkResponseForException(
        ...
        if (cause instanceof Exceptions.AppendSerializationError) {
            ...
            IntStream.range(0, payload.getPayloadIndexes().size())
                    .forEach(index -> {
                        sinkResponse.addErrors(payload.getInputIndex(index),
                                new ErrorInfo(new Exception(cause), ErrorType.SINK_5XX_ERROR));
            ...
            if (BigQueryStorageResponseParser.shouldRetry(status)) {
                ...
                IntStream.range(0, payload.getPayloadIndexes().size())
                        .forEach(index -> {
                            sinkResponse.addErrors(payload.getInputIndex(index),
                                    new ErrorInfo(new Exception(cause), ErrorType.SINK_4XX_ERROR));
                            instrumentErrors(status.getCode());
                        });
            }
        }
