...
import org.raystack.depot.bigquery.exception.BQPartitionKeyNotSpecified;
...
public class BigQueryTableDefinition {
    ...
    private TimePartitioning getPartitionedTableDefinition(Schema schema) {
        ...
        if (tablePartitionKey == null) {
            throw new BQPartitionKeyNotSpecified(
                    String.format("Partition key not specified for the table: %s", bqConfig.getTableName()));
        }
        ...
        if (!partitionFieldOptional.isPresent()) {
            throw new BQPartitionKeyNotSpecified(
                    String.format("Partition key %s is not present in the schema", tablePartitionKey));
        }
        Field partitionField = partitionFieldOptional.get();
        if (partitionField.getType() == LegacySQLTypeName.TIMESTAMP
                || partitionField.getType() == LegacySQLTypeName.DATE) {
    ...
    private Clustering getClusteredTableDefinition(Schema schema) {
        if (bqConfig.getTableClusteringKeys().isEmpty()) {
            throw new BQClusteringKeysException(
                    String.format("Clustering key not specified for the table: %s", bqConfig.getTableName()));
        }
        List<String> columnNames = bqConfig.getTableClusteringKeys();
        if (columnNames.size() > MAX_CLUSTERING_KEYS) {
            throw new BQClusteringKeysException(
                    String.format("Max number of columns for clustering is %d", MAX_CLUSTERING_KEYS));
        }
        ...
        if (!fieldNames.containsAll(columnNames)) {
            throw new BQClusteringKeysException(String.format(
                    "One or more column names specified %s not exist on the schema or a nested type which is not supported for clustering",
                    columnNames));
        }
