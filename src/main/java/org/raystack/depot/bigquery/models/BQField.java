...
public class BQField {
    ...
    private LegacySQLTypeName getType(ProtoField protoField) {
        ...
        if (typeFromFieldName == null) {
            throw new BQSchemaMappingException(
                    String.format("No type mapping found for field: %s, fieldType: %s, typeName: %s",
                            protoField.getName(), protoField.getType(), protoField.getTypeName()));
        }
